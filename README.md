...
项目使用非 `public` schema（如 `init_project`），配置方式：

1. `DATABASE_URL` 中指定 `schema` 参数：
   ```
   DATABASE_URL="postgresql://user:pass@localhost:5432/db?schema=init_project"
   ```
...
```bash
# 数据库连接（需包含 schema 参数）
DATABASE_URL="postgresql://用户名:密码@localhost:5432/数据库名?schema=init_project"
